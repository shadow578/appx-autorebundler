...
function Invoke-UserScript([string] $ExtractedBundleDir, [string[]] $ExtractedPackDirs) {
    ...
    function Update-AppXManifest([Parameter(ValueFromPipeline = $true)][string] $ManifestContents) {
        # name
        $ManifestContents = $ManifestContents -replace "Name=`"SOMEONE`"", "Name=`"NOT_SOMEONE`""

        # publisher
        $ManifestContents = $ManifestContents -replace "Publisher=`"CN=SOMEBODY`"", "Publisher=`"$(Get-DummySubject)`""
